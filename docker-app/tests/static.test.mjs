...
test("graphical interface contains the required workflows", async () => {
  ...
  for (const id of ["companiesButton", "ipamButton", "inventoryButton", "radiosButton", "onlineReportsButton", "companySelect", "spaceSelect", "companyDialog", "companyContacts", "companyConnections", "prefixDialog", "hostDialog", "hostPorts", "hostMonitorEnabled", "hostBackupEnabled", "hostSaveButton", "hostSaveCloseButton", "hostPrevButton", "hostNextButton", "devicePortsList", "usersDialog", "spaceAccessList", "backupsDialog", "backupIntervalDays", "configBackupsDialog", "configBackupTargets", "settingsDialog", "appearanceFont", "mapperUrl", "importDialog", "trashDialog", "aboutDialog", "mikrotikScriptDialog", "mapDialog", "toolMenu"]) {
  ...
  assert.match(script, /emsipam-client:\/\/open/);
...
test("browser modules are served with a JavaScript MIME type", async () => {
  ...
  assert.match(html, /app\.js\?v=0\.5\.2/);
  assert.match(html, /<html[^>]+data-theme=["']dark["']/);
  assert.match(script, /subnet-model\.mjs\?v=0\.5\.2/);
});
