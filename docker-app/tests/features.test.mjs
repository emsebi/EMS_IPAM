...
test("schema keeps companies, IP, scoped access and topology in one database", async () => {
  ...
  for (const table of ["company_contacts", "company_connections", "app_settings", "user_space_access", "user_module_access", "device_ports", "topology_maps", "topology_nodes", "topology_links"]) {
...
test("destructive and scoped workflows keep their safety guards", async () => {
  ...
  assert.match(server, /async function discoverModules/);
  assert.match(server, /module\.json/);
  assert.match(server, /installedModules/);
  assert.match(server, /EMS_MODULES_DIR/);
  assert.match(server, /MODULE_CATALOG/);
  assert.match(server, /user_module_access/);
